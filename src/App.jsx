...
const c = {
  ...
  meta: { display: "flex", gap: 12, fontSize: 12, color: "#666", marginTop: 14 },
  ...
  card: { backgroundColor: "#0f0f0f", border: "1px solid #222", borderRadius: 14, overflow: "hidden" },
...
export default function CannesTracker() {
  ...
  const filtered = activeDay === "All" ? events : events.filter(e => e.date === activeDay);
  const goingCount = events.filter(e => e.status === "going").length;
  ...
  const handleAdd = async () => {
    ...
    const result = await insertEvent(form);
        ...
        body: JSON.stringify({
          ...
          messages: [{
            ...
            content: [
              ...
              { type: "text", text: `You are reading an event flyer. Extract event details and return ONLY valid JSON, no markdown, no explanation. Use this exact structure:
{
  "name": "event name",
  "venue": "venue name",
  "date": "June 22 or June 23 or June 24 or June 25 or June 26 (pick closest match, default June 22 if unclear)",
  ...
  "notes": "performers, hosts, dress code, any other key details"
}` }
            ]
          }]
        })
    ...
    <>
      <style>{`
        * { box-sizing: border-box; }
        body, html, #root { background-color: #000 !important; }
      ...
      <div style={c.root}>
        <div style={c.header}>
          ...
          <p style={c.sub}>Your event radar</p>
          <div style={c.meta}>
            <span><span style={{ color: "#fff", fontWeight: 700 }}>{events.length}</span> events</span>
            <span style={{ color: "#333" }}>·</span>
            <span><span style={{ color: "#C8F97A", fontWeight: 700 }}>{goingCount}</span> confirmed</span>
          </div>
        </div>
        ...
        <div style={c.list}>
          {loading && <div style={c.loadingBox}>Loading events...</div>}
          {!loading && filtered.length === 0 && <div style={c.empty}>No events yet. Add one below!</div>}
          {filtered.map(event => {
            const isOpen = expandedId === event.id;
            return (
              <div key={event.id} style={c.card}>
                  ...
                  <div style={c.cardRow}>
                    ...
                    <div style={c.cardRight}>
                      <button style={c.statusBtn(event.status || "maybe")}
                        onClick={e => { e.stopPropagation(); cycleStatus(event.id, event.status || "maybe"); }}>
                ...
                {isOpen && (
                  <div style={c.expandArea}>
                    ...
                    <button style={c.removeBtn} onClick={() => deleteEvent(event.id)}>Remove ✕</button>
                  </div>
                )}
              </div>
            );
        ...
        <div style={c.addWrap}>
          ...
          {mode === "manual" && (
            <div style={c.form}>
              ...
              <div style={c.btnRow}>
                <button style={c.cancelBtn} onClick={() => { setMode("list"); setForm(EMPTY_FORM); }}>Cancel</button>
...
export default function CannesTracker() {
  ...
  const [activeDay, setActiveDay] = useState("All");
  const [mode, setMode] = useState("list"); // list | manual | flyer
  const [form, setForm] = useState(EMPTY_FORM);
  const [expandedId, setExpandedId] = useState(null);
  const [flyerImg, setFlyerImg] = useState(null);
  const [scanning, setScanning] = useState(false);
  ...
  const cycleStatus = (id) => {
    const next = { going: "maybe", maybe: "pass", pass: "going" };
    setEvents(prev => prev.map(e => e.id === id ? { ...e, status: next[e.status] } : e));
  };
        ...
        body: JSON.stringify({
          ...
          messages: [{
            ...
            content: [
              ...
              {
                type: "text",
                text: `You are reading an event flyer. Extract event details and return ONLY valid JSON, no markdown, no explanation. Use this exact structure:
{
  "name": "event name",
  "venue": "venue name",
  "date": "June 22 or June 23 or June 24 or June 25 or June 26 (pick closest match from these options, default June 22 if unclear)",
  "time": "start–end time or empty string",
  "vibe": "pick one: Afrobeats / Party, Networking, Day Party, Rooftop, Culture House, Brand Event, Club, Other",
  "tickets": "ticket website or empty string",
  "tables": "table reservation contact or empty string",
  ...
  const FormFields = () => (
    <div style={c.form}>
      ...
      <div style={c.btnRow}>
        <button style={c.cancelBtn} onClick={() => { setMode("list"); setForm(EMPTY_FORM); }}>Cancel</button>
        <button style={c.submitBtn} onClick={handleAdd}>Add Event</button>
      </div>
    </div>
  );
  ...
  return (
    <>
      ...
      <div style={c.root}>
        ...
        {/* EVENT LIST */}
        <div style={c.list}>
          {filtered.length === 0 && <div style={c.empty}>No events on {activeDay}.</div>}
          {filtered.map(event => {
            const isOpen = expandedId === event.id;
            return (
              <div key={event.id} style={c.card}>
                <div style={c.cardInner} onClick={() => setExpandedId(isOpen ? null : event.id)}>
                  <div style={c.cardRow}>
                    ...
                    <div style={c.cardRight}>
                      <button style={c.statusBtn(event.status)}
                        onClick={e => { e.stopPropagation(); cycleStatus(event.id); }}>
        ...
        <div style={c.addWrap}>
          ...
          {mode === "list" && (
            <>
              <button style={c.flyerBtn} onClick={() => setMode("flyer")}>
                📸 Scan a Flyer
              </button>
              <button style={c.addTrigger} onClick={() => setMode("manual")}>
                + Add Event Manually
              </button>
            </>
          )}
